...
expInfo = {'participant': '', 'session': '001', 'HPColor': 'black', 'verifyEstimate': False}
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/brandonchen93/EASTO/Paradigm/Master/Spatial_V4/Practice/SEA_PracticeQ_lastrun.py',
...
# Initialize components for Routine "quest_estimate_test"
quest_estimate_testClock = core.Clock()
if expInfo['verifyEstimate']: 
...
exp_instr_txt = ('''To help you identify which image will have meaningful content and which will not, both stimuli will be contained in a placeholder.
\n\n\n\n\n\n\n\n
The {H_color} lined placeholder indicates that a meaningful image will appear within the placeholder, The {L_color} lined placeholder indicates that a noise image will appear within the placeholder. 
\n
...
probe_cue_rec_2 = visual.ImageStim(
    ...
    flipHoriz=False, flipVert=False,
    texRes=512, interpolate=True, depth=-2.0)
rec_resp_exemp = keyboard.Keyboard()
...
#Initialize counter for recognition response 
num_recognized = 0
...
# ------Prepare to start Routine "quest_estimate_test"-------
continueRoutine = True
# update component parameters for each repeat
# keep track of which components have finished
quest_estimate_testComponents = []
for thisComponent in quest_estimate_testComponents:
...
quest_estimate_testClock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
frameN = -1

# -------Run Routine "quest_estimate_test"-------
while continueRoutine:
    # get current time
    t = quest_estimate_testClock.getTime()
    tThisFlip = win.getFutureFlipTime(clock=quest_estimate_testClock)
...
# -------Ending Routine "quest_estimate_test"-------
for thisComponent in quest_estimate_testComponents:
    if hasattr(thisComponent, "setAutoDraw"):
        thisComponent.setAutoDraw(False)
# the Routine "quest_estimate_test" was not non-slip safe, so reset the non-slip timer
routineTimer.reset()
...
for thisExemp in exemps:
    ...
    exemplar = np.array(img  * params['filter'] * 0.6)
...
placholder_example = data.TrialHandler(nReps=1, method='random', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('exp_exemp.xlsx', selection='0:2'),
...
rec_resp_exemp.keys = []
rec_resp_exemp.rt = []
_rec_resp_exemp_allKeys = []
# keep track of which components have finished
q_rec_exempComponents = [q_recognition_3, probe_cue_rec_2, rec_resp_exemp]
...
while continueRoutine:
    ...
    # *rec_resp_exemp* updates
    waitOnFlip = False
    if rec_resp_exemp.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        rec_resp_exemp.frameNStart = frameN  # exact frame index
        rec_resp_exemp.tStart = t  # local t and not account for scr refresh
        rec_resp_exemp.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(rec_resp_exemp, 'tStartRefresh')  # time at next scr refresh
        rec_resp_exemp.status = STARTED
        # keyboard checking is just starting
        waitOnFlip = True
        win.callOnFlip(rec_resp_exemp.clock.reset)  # t=0 on next screen flip
        win.callOnFlip(rec_resp_exemp.clearEvents, eventType='keyboard')  # clear events on next screen flip
    if rec_resp_exemp.status == STARTED and not waitOnFlip:
        theseKeys = rec_resp_exemp.getKeys(keyList=['7', '8'], waitRelease=False)
        _rec_resp_exemp_allKeys.extend(theseKeys)
        if len(_rec_resp_exemp_allKeys):
            rec_resp_exemp.keys = _rec_resp_exemp_allKeys[-1].name  # just the last key pressed
            rec_resp_exemp.rt = _rec_resp_exemp_allKeys[-1].rt
            # a response ends the routine
            continueRoutine = False
...
# check responses
if rec_resp_exemp.keys in ['', [], None]:  # No response was made
    rec_resp_exemp.keys = None
thisExp.addData('rec_resp_exemp.keys',rec_resp_exemp.keys)
if rec_resp_exemp.keys != None:  # we had a response
    thisExp.addData('rec_resp_exemp.rt', rec_resp_exemp.rt)
thisExp.addData('rec_resp_exemp.started', rec_resp_exemp.tStartRefresh)
thisExp.addData('rec_resp_exemp.stopped', rec_resp_exemp.tStopRefresh)
thisExp.nextEntry()
...
for level, condition in trials:
    ...
    # update component parameters for each repeat
    #Taking a break every 40 trials
    break_freq = 40
...
round(120 - t, ndigits = 1))
    ...
    if expInfo['HPColor'] == 'black':
        HP_color = [-1.0,-1.0,-1.0]
        LP_color = [1.0,1.0,1.0]
    elif expInfo['HPColor'] == 'white':
        LP_color = [-1.0,-1.0,-1.0]
        HP_color =  [1.0,1.0,1.0]
    ...
    minC = 0.001 
    if expInfo['verifyEstimate']:
        maxC = quest_estimates[stim_img[:13]] 
    else:
        maxC = 10**level
    
    #Presenting Stimuli for 300 ms for practice instead of 66 ms unless verifying threshold
    ...
    # keep track of which components have finished
    blank_postComponents = [post_stim_fix, place_left_5, place_right_5, long_back_R_5, hi_back_R_5, hi_back_L_5, long_back_L_5]
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if place_left_5.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > place_left_5.tStartRefresh + .4-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if place_right_5.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > place_right_5.tStartRefresh + .4-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if long_back_R_5.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > long_back_R_5.tStartRefresh + .4-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if hi_back_R_5.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > hi_back_R_5.tStartRefresh + .4-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if hi_back_L_5.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > hi_back_L_5.tStartRefresh + .4-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if long_back_L_5.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > long_back_L_5.tStartRefresh + .4-frameTolerance:
                # keep track of stop time/frame for later
    ...
    trials.addOtherData('place_left_5.started', place_left_5.tStartRefresh)
    trials.addOtherData('place_left_5.stopped', place_left_5.tStopRefresh)
    trials.addOtherData('place_right_5.started', place_right_5.tStartRefresh)
    trials.addOtherData('place_right_5.stopped', place_right_5.tStopRefresh)
    trials.addOtherData('long_back_R_5.started', long_back_R_5.tStartRefresh)
    trials.addOtherData('long_back_R_5.stopped', long_back_R_5.tStopRefresh)
    trials.addOtherData('hi_back_R_5.started', hi_back_R_5.tStartRefresh)
    trials.addOtherData('hi_back_R_5.stopped', hi_back_R_5.tStopRefresh)
    trials.addOtherData('hi_back_L_5.started', hi_back_L_5.tStartRefresh)
    trials.addOtherData('hi_back_L_5.stopped', hi_back_L_5.tStopRefresh)
    trials.addOtherData('long_back_L_5.started', long_back_L_5.tStartRefresh)
    trials.addOtherData('long_back_L_5.stopped', long_back_L_5.tStopRefresh)
    ...
    # keep track of which components have finished
    q_catComponents = [q_category, cat_resp, probe_cue_cat, place_left_6, place_right_6, long_back_R_6, hi_back_R_6, hi_back_L_6, long_back_L_6]
    for thisComponent in q_catComponents:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if place_left_6.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > place_left_6.tStartRefresh + 3-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if place_right_6.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > place_right_6.tStartRefresh + 3-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if long_back_R_6.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > long_back_R_6.tStartRefresh + 3-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if hi_back_R_6.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > hi_back_R_6.tStartRefresh + 3-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if hi_back_L_6.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > hi_back_L_6.tStartRefresh + 3-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if long_back_L_6.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > long_back_L_6.tStartRefresh + 3-frameTolerance:
                # keep track of stop time/frame for later
    ...
    thisExp.addData('objetive_category', correct_category)
    ...
    trials.addOtherData('place_left_6.started', place_left_6.tStartRefresh)
    trials.addOtherData('place_left_6.stopped', place_left_6.tStopRefresh)
    trials.addOtherData('place_right_6.started', place_right_6.tStartRefresh)
    trials.addOtherData('place_right_6.stopped', place_right_6.tStopRefresh)
    trials.addOtherData('long_back_R_6.started', long_back_R_6.tStartRefresh)
    trials.addOtherData('long_back_R_6.stopped', long_back_R_6.tStopRefresh)
    trials.addOtherData('hi_back_R_6.started', hi_back_R_6.tStartRefresh)
    trials.addOtherData('hi_back_R_6.stopped', hi_back_R_6.tStopRefresh)
    trials.addOtherData('hi_back_L_6.started', hi_back_L_6.tStartRefresh)
    trials.addOtherData('hi_back_L_6.stopped', hi_back_L_6.tStopRefresh)
    trials.addOtherData('long_back_L_6.started', long_back_L_6.tStartRefresh)
    trials.addOtherData('long_back_L_6.stopped', long_back_L_6.tStopRefresh)
    ...
    # keep track of which components have finished
    q_recComponents = [q_recognition, rec_resp, probe_cue_rec, place_left_7, place_right_7, long_back_R_7, hi_back_R_7, hi_back_L_7, long_back_L_7]
    for thisComponent in q_recComponents:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if place_left_7.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > place_left_7.tStartRefresh + 3-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if place_right_7.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > place_right_7.tStartRefresh + 3-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if long_back_R_7.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > long_back_R_7.tStartRefresh + 3-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if hi_back_R_7.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > hi_back_R_7.tStartRefresh + 3-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if hi_back_L_7.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > hi_back_L_7.tStartRefresh + 3-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if long_back_L_7.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > long_back_L_7.tStartRefresh + 3-frameTolerance:
                # keep track of stop time/frame for later
    ...
    for thisComponent in q_recComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    #Calculate proportion recognized and save response
    if rec_resp.keys == '7':
    ...
    if probe_loc == 'L':
        win.getMovieFrame()
        win.saveMovieFrames('SEAQRecL.png')
    elif probe_loc == 'R':
        win.getMovieFrame()
        win.saveMovieFrames('SEAQRecR.png')
    ...
    trials.addOtherData('place_left_7.started', place_left_7.tStartRefresh)
    trials.addOtherData('place_left_7.stopped', place_left_7.tStopRefresh)
    trials.addOtherData('place_right_7.started', place_right_7.tStartRefresh)
    trials.addOtherData('place_right_7.stopped', place_right_7.tStopRefresh)
    trials.addOtherData('long_back_R_7.started', long_back_R_7.tStartRefresh)
    trials.addOtherData('long_back_R_7.stopped', long_back_R_7.tStopRefresh)
    trials.addOtherData('hi_back_R_7.started', hi_back_R_7.tStartRefresh)
    trials.addOtherData('hi_back_R_7.stopped', hi_back_R_7.tStopRefresh)
    trials.addOtherData('hi_back_L_7.started', hi_back_L_7.tStartRefresh)
    trials.addOtherData('hi_back_L_7.stopped', hi_back_L_7.tStopRefresh)
    trials.addOtherData('long_back_L_7.started', long_back_L_7.tStartRefresh)
    trials.addOtherData('long_back_L_7.stopped', long_back_L_7.tStopRefresh)
    thisExp.nextEntry()
...
routineTimer.reset()
#Show number of proportion of recognized images 
proportion_rec = num_recognized / trials.nTotal
print('Proportion Recognized: {:.0%}.'.format(proportion_rec))
