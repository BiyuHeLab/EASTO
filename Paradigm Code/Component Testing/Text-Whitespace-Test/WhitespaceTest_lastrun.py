...
expName = 'WhitespaceTest'  # from the Builder filename that created this script
expInfo = {'participant': ''}
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/brandonchen93/Downloads/Attention-Expectation/Object/WhitespaceTest_lastrun.py',
...
# Initialize components for Routine "q_catgen"
q_catgenClock = core.Clock()
...
trials = data.TrialHandler(nReps=100, method='sequential', 
    ...
    trialList=data.importConditions('blocksTrunc.xlsx', selection='1'),
...
for thisTrial in trials:
    ...
    cat_trunc = ['F','A', 'O', 'H']
    cat_full = ['Face', 'Animal', 'Object', 'House'] 
    
    block_catmap = dict(zip(cat_trunc,cat_full))
    
    probe_loc = 'c1'
    if probe_loc == 'c1':
        probe_cue_txt = cat_comb[0]
        
    elif probe_loc == 'c2':
        probe_cue_txt = cat_comb[1]
    
    # Randomize Order of subcategory possible responses
    #Initialize cat components
    
    if probe_cue_txt == 'F':
        cats = ['Male', 'Female', 'X'] 
    
    elif probe_cue_txt == 'H':
        cats= ['House', 'Building', 'X'] 
    
    elif probe_cue_txt =='A':
        cats = ['Mammal', 'Non-Mammal', 'X'] 
    
    
    elif probe_cue_txt == 'O':
        cats = ['Handheld', 'Non-Handheld', 'X'] 
    
    shuffle(cats)
    cat_question = (
                    u'Image Category:\n\n\n\n\n'
                    u'{cats1:\u00a0^15} {cats2:\u00a0^15} {cats3:\u00a0^15}\n\n\n'
                    u'{b1:\u00a0^15} {b2:\u00a0^15} {b3:\u00a0^15}'.format(cats1 = cats[0].encode().decode('utf-8'), 
                                                               cats2 = cats[1].encode().decode('utf-8'), 
                                                               cats3 = cats[2].encode().decode('utf-8'), b1 = u'1',b2 = u'2',b3 = u'3', 
                                                                ))
    ...
    q_catComponents = [q_category, cat_resp]
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if q_category.status == STARTED:
            ...
            if tThisFlipGlobal > q_category.tStartRefresh + 500-frameTolerance:
        ...
        if cat_resp.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > cat_resp.tStartRefresh + 500-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if cat_resp.status == STARTED and not waitOnFlip:
            theseKeys = cat_resp.getKeys(keyList=['6', '7', '8'], waitRelease=False)
    ...
    #
    ## Set correct answer dependent on if target is congruent with probe
    #if probe_loc == 'c1':
    #    corAns = button_map[probe_cue_txt] # Index 2nd letter after " images/" in target
    #    
    #elif probe_loc != 'c1':
    #    corAns = button_map['X'] # Index first letter after " images/" in target
    #
    #trials.addData('cat_correct', cat_map[corAns])
    #
    #if cat_resp.keys:
    #    trials.addData('cat_select', cat_map[cat_resp.keys[-1]])
    #
    #
    #    
    #
    ...
    # ------Prepare to start Routine "q_catgen"-------
    continueRoutine = True
    routineTimer.add(500.000000)
    # update component parameters for each repeat
    # Use routine if participant answered X 
    if not cat_resp.keys or cat_map[cat_resp.keys[-1]] != 'X':
        continueRoutine = False
    
    # Randomize Order of FAOH 
    cats_gen = [block_catmap[cat_comb[0]], block_catmap[cat_comb[1]]] 
    shuffle(cats_gen)
    cat_question = (
                    u'Image Category:\n\n\n\n\n'
                    u'{cats1:\u00a0^15} {cats2:\u00a0^15} \n\n\n'
                    u'{b1:\u00a0^15} {b2:\u00a0^15}'.format(cats1 = cats_gen[0].encode().decode('utf-8'), 
                                                               cats2 = cats_gen[1].encode().decode('utf-8'), 
                                                               b1 = u'2',b2 = u'3' 
                                                                ))
    ...
    _cat_resp_2_allKeys = []
    # keep track of which components have finished
    q_catgenComponents = [q_category_2, cat_resp_2]
    for thisComponent in q_catgenComponents:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    t = 0
    _timeToFirstFrame = win.getFutureFlipTime(clock="now")
    q_catgenClock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
    frameN = -1
    
    # -------Run Routine "q_catgen"-------
    while continueRoutine and routineTimer.getTime() > 0:
        # get current time
        t = q_catgenClock.getTime()
        tThisFlip = win.getFutureFlipTime(clock=q_catgenClock)
        tThisFlipGlobal = win.getFutureFlipTime(clock=None)
        frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
        # update/draw components on each frame
        ...
        if q_category_2.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > q_category_2.tStartRefresh + 500-frameTolerance:
                # keep track of stop time/frame for later
                q_category_2.tStop = t  # not accounting for scr refresh
                q_category_2.frameNStop = frameN  # exact frame index
                win.timeOnFlip(q_category_2, 'tStopRefresh')  # time at next scr refresh
                q_category_2.setAutoDraw(False)
        ...
        if cat_resp_2.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > cat_resp_2.tStartRefresh + 500-frameTolerance:
                # keep track of stop time/frame for later
                cat_resp_2.tStop = t  # not accounting for scr refresh
                cat_resp_2.frameNStop = frameN  # exact frame index
                win.timeOnFlip(cat_resp_2, 'tStopRefresh')  # time at next scr refresh
                cat_resp_2.status = FINISHED
        if cat_resp_2.status == STARTED and not waitOnFlip:
            theseKeys = cat_resp_2.getKeys(keyList=['7', '8'], waitRelease=False)
    ...
    #
    #buttons2 = ['7','8']
    #categories2 = [c[0] for c in cats_gen]
    #
    #button_map2 = dict(zip(categories2,buttons2))
    #cat_map2  = dict(zip(buttons2,categories2))
    #
    ##Correct answer is based on the general category of the target image 
    #corAns = button_map2[probe_cue_txt] # Index first letter after " images/" in target
    #
    ##Add correct response & selected response to stored data
    #trials.addData('cat_correctgen', cat_map2[corAns])
    #
    #
    #if cat_resp_2.keys: #If response was made 
    #    trials.addData('cat_selectgen', cat_map2[cat_resp_2.keys[-1]])
    #
    #
    #    
    #
    trials.addData('q_category_2.started', q_category_2.tStartRefresh)
    trials.addData('q_category_2.stopped', q_category_2.tStopRefresh)
    # check responses
    if cat_resp_2.keys in ['', [], None]:  # No response was made
        cat_resp_2.keys = None
    trials.addData('cat_resp_2.keys',cat_resp_2.keys)
    if cat_resp_2.keys != None:  # we had a response
        trials.addData('cat_resp_2.rt', cat_resp_2.rt)
    trials.addData('cat_resp_2.started', cat_resp_2.tStartRefresh)
    trials.addData('cat_resp_2.stopped', cat_resp_2.tStopRefresh)
    ...
    if cat_resp.keys and cat_map[cat_resp.keys[-1]] == 'X': 
        continueRoutine = False
        
    
    
    q_recognition.setText(rec_question)
    rec_resp.keys = []
    rec_resp.rt = []
    _rec_resp_allKeys = []
    # keep track of which components have finished
    q_recComponents = [q_recognition, rec_resp]
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if q_recognition.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > q_recognition.tStartRefresh + 500-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if rec_resp.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > rec_resp.tStartRefresh + 500-frameTolerance:
                # keep track of stop time/frame for later
