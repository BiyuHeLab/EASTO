# This code generates visualization of pupil dimater changes in Exp 3.
# We compare:
# pipul diameter dynamics in trials where stimulus at the attended location
# matches vs violates the expeted stimulus type (real/ scrambled)    
...
for subj in ['P57', 'P58', 'P59', 'P63']:
    ...
    for block in ['LE', 'GE']:
        ...
        if block =='LE':
            block_mask = (subj_df_sorted['exp_color']!='Neutral') & (subj_df_sorted['brokeFixation']==0)
            block_df = subj_df_sorted[block_mask] 
            
        
            block_df['expectation match'] = np.where(
            (
                (block_df['att_loc'] == 'L')
                & (block_df['exp_loc'] == 'L')
                & (~block_df['stim_left'].str.contains('scram', na=False))
            )
            | (
                (block_df['att_loc'] == 'R')
                & (block_df['exp_loc'] == 'R')
                & (~block_df['stim_right'].str.contains('scram', na=False))
            )
            | (
                (block_df['att_loc'] == 'L')
                & (block_df['exp_loc'] == 'R')
        ...
        timestamp = Period_Interest[block_mask.to_numpy()].reset_index(drop=True)
        ...
        for idx in range(len(block_df)):
            ...
            for t in win_onsets:
                ...
                rate_rows.append({
                        ...
                        "subject": subj,
                        "block type": block,
                        "trial type": block_df.loc[idx,'expectation match'],
                        "time_ms": t,
...
plot_df = (rate_all_trials.groupby(["subject", "block type", "trial type", "time_ms"])["pupil size"]
    .mean().reset_index())
...
for block in ["LE", "GE"]:
    ...
    ax = sns.lineplot(data=block_df, x="time_ms", y="pupil size",
        hue='trial type', errorbar="se", lw=1)
    #ax.set_ylim([-1, 1.25])
    ...
    plt.yticks([-0.5, 0.0, 0.5, 1.0],
               ["-0.5", "0.0", "0.5", "1.0"], fontsize=8)
    plt.xlabel("Time relative to Stimulus Onset (ms)", fontsize=8)
    plt.ylabel("Normalized Pupil dilation", fontsize=8)
    plt.title(f"{block} BLOCK", fontsize=10, fontweight='bold')
    plt.legend(title="Trial type", fontsize=8, title_fontsize=8,
               loc='center')
    plt.tight_layout()
    plt.show()
