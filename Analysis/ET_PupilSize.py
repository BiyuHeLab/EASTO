...
for subj in ['P57', 'P58', 'P59', 'P63']:
    ...
    for block in ['LE', 'GE']:
        for trial_type in ['valid', 'invalid']:
        # --- build block-specific trial data ---
            if block =='LE' and trial_type=='valid':
                cond_mask = (subj_df_sorted['exp_color']!='Neutral') \
                    & (subj_df_sorted['brokeFixation']==0) \
                    & subj_df_sorted['trial_type'].isin(['E+A+', 'E+A-'])
            elif block =='LE' and trial_type=='invalid':
                cond_mask = (subj_df_sorted['exp_color']!='Neutral') \
                    & (subj_df_sorted['brokeFixation']==0) \
                    & subj_df_sorted['trial_type'].isin(['E-A+', 'E-A-'])
            elif block =='GE' and trial_type=='valid':
                cond_mask = (subj_df_sorted['exp_color']=='Neutral') \
                    & (subj_df_sorted['brokeFixation']==0) \
                    & subj_df_sorted['trial_type'].isin(['E+A+', 'E+A-'])
            elif block =='GE' and trial_type=='invalid':
                cond_mask = (subj_df_sorted['exp_color']=='Neutral') \
                    & (subj_df_sorted['brokeFixation']==0) \
                    & subj_df_sorted['trial_type'].isin(['E-A+', 'E-A-'])
...
for block in ["LE", "GE"]:
    ...
    ax = sns.lineplot(data=block_df, x="time_ms", y="pupil size",
        hue='trial type', errorbar="se", lw=2)
    ax.set_ylim([-1, 1.25])
