...
def ParseEyeLinkAsc(elFilename):
    ...
    for iLine in range(nLines):
        ...
        elif fileTxt0[iLine].split()[0][0].isdigit() and len(fileTxt0[iLine].split()) >= 6: 
            lineType[iLine] = 'SAMPLE' 
        else:
        ...
        #    iStartRec = iLine+1
        if '!CAL' in fileTxt0[iLine] or fileTxt0[iLine].startswith('START'):
            iStartRec = iLine + 1
    print('Done! Took %f seconds.'%(time.time()-t))
...
for subj in subjects:
    elFilename = glob(join(DataDir, expt_version,
                           subj, paradigm, 'Main', 'edfData', "*.asc"))[0]
    subjdir = join(DataDir, expt_version, subj,
               paradigm, 'Main', 'edfData')
    
    try:
        print(f'Processing subject {subj}...')
        dfRec, dfMsg, dfFix, dfSacc, dfBlink, dfSamples = ParseEyeLinkAsc(elFilename)
    
        with open(join(subjdir, f"{subj}_eye_tracking_data.pkl"), "wb") as f:
            pickle.dump({
                'dfRec': dfRec,
                'dfMsg': dfMsg,
                'dfFix': dfFix,
        ...
        print(f"Error occurred while processing: {subj}")
